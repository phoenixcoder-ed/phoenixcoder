...
class RequestIdFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        if not request_id:
            # 如果没有请求ID，生成一个简单的标识
            record.request_id = f"oidc-{datetime.now().strftime('%H%M%S')}"
        return True
...
def setup_logging(
    ...
    debug_file_handler = TimedRotatingFileHandler(
        os.path.join(LOG_DIR, "oidc-debug.log"),
        when='midnight',
        interval=1,
        backupCount=7,  # 保留7天
        encoding="utf-8"
    ...
    app_file_handler = RotatingFileHandler(
        os.path.join(LOG_DIR, "oidc-app.log"),
    ...
    error_file_handler = RotatingFileHandler(
        os.path.join(LOG_DIR, "oidc-error.log"),
    ...
    # 设置文件处理器格式和过滤器
    for handler in [debug_file_handler, app_file_handler, error_file_handler, auth_file_handler]:
    ...
    root_logger.addHandler(auth_file_handler)
    
    # 创建应用专用logger
    app_logger = logging.getLogger('phoenixcoder.oidc')
    
    # 记录初始化信息
    app_logger.info(f"OIDC日志系统初始化完成")
...
def get_logger(name: str = None) -> logging.Logger:
    ...
    """
    if name:
        return logging.getLogger(f'phoenixcoder.oidc.{name}')
    return logging.getLogger('phoenixcoder.oidc')

def get_auth_logger() -> logging.Logger:
    """获取认证专用logger"""
    return logging.getLogger('phoenixcoder.oidc.auth')
...
def auth_info(msg, *args, **kwargs):
    """认证信息日志"""
    auth_logger = get_auth_logger()
    auth_logger.info(msg, *args, **kwargs)

def auth_warning(msg, *args, **kwargs):
    """认证警告日志"""
    auth_logger = get_auth_logger()
    auth_logger.warning(msg, *args, **kwargs)

def auth_error(msg, *args, **kwargs):
    """认证错误日志"""
    auth_logger = get_auth_logger()
    auth_logger.error(msg, *args, **kwargs)