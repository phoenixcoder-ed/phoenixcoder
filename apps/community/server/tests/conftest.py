...
def mock_database_settings():
    """自动模拟数据库设置"""
    with patch('config.settings.DatabaseSettings') as mock_settings:
        mock_instance = Mock()
...
def mock_jwt_settings():
    """自动模拟JWT设置"""
    with patch('config.settings.SecuritySettings') as mock_settings:
        mock_instance = Mock()
        mock_instance.jwt_secret = 'test_secret_key_for_testing_only'
