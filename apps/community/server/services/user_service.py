...
class UserService:
    ...
    async def get_user_stats(self) -> UserStats:
        ...
        return UserStats(
            ...
            newUsersToday=stats.get("new_users_today", 0),
            newUsersThisWeek=stats.get("new_users_this_week", 0),
            newUsersThisMonth=stats.get("new_users_this_month", 0)
        )
    ...
    def _build_search_conditions(self, filters: UserSearchFilters) -> Dict[str, Any]:
        ...
        if filters.userType:
            conditions["user_type"] = filters.userType
        ...
        if filters.minExperience is not None:
            conditions["min_experience"] = filters.minExperience
        
        if filters.maxExperience is not None:
            conditions["max_experience"] = filters.maxExperience
        
        if filters.minHourlyRate is not None:
            conditions["min_hourly_rate"] = filters.minHourlyRate
        
        if filters.maxHourlyRate is not None:
            conditions["max_hourly_rate"] = filters.maxHourlyRate
