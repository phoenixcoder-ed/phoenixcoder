...
class TaskService:
    ...
    async def get_task_stats(self, user_id: Optional[str] = None) -> TaskStats:
        ...
        return TaskStats(
            ...
            cancelledTasks=stats.get("cancelled_tasks", 0),
            tasksToday=stats.get("tasks_today", 0),
            tasksThisWeek=stats.get("tasks_this_week", 0),
            tasksThisMonth=stats.get("tasks_this_month", 0),
            averageBudget=stats.get("average_budget", 0.0),
            totalBudget=stats.get("total_budget", 0.0)
        )
    ...
    def _validate_budget(
        ...
        if (budget_min is not None and budget_max is not None and 
    ...
    def _build_search_conditions(self, filters: TaskSearchFilters) -> Dict[str, Any]:
        ...
        if filters.taskType:
            conditions["task_type"] = filters.taskType
        
        if filters.priority:
            conditions["priority"] = filters.priority
        
        if filters.minBudget is not None:
            conditions["min_budget"] = filters.minBudget
        
        if filters.maxBudget is not None:
            conditions["max_budget"] = filters.maxBudget
        ...
        if filters.deadlineBefore:
            conditions["deadline_before"] = filters.deadlineBefore
        
        if filters.deadlineAfter:
            conditions["deadline_after"] = filters.deadlineAfter
        
        if filters.searchText:
            conditions["search_text"] = filters.searchText
        
        if filters.employerId:
            conditions["employer_id"] = filters.employerId
        
        if filters.assigneeId:
            conditions["assignee_id"] = filters.assigneeId
