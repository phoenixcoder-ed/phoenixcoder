...
class DatabaseSettings(BaseSettings):
    """数据库配置"""
    host: str = Field(default="nonexistent-host")
    port: int = Field(default=5432)
    db: str = Field(default="phoenixcoder")
...
class OIDCSettings(BaseSettings):
    """OIDC配置"""
    issuer: str = Field(default="http://localhost:8080", description="OIDC发行者URL", env="OIDC__ISSUER")
    client_id: str = Field(default="phoenixcoder", description="OIDC客户端ID", env="OIDC__CLIENT_ID")
    client_secret: str = Field(default="secret", description="OIDC客户端密钥", env="OIDC__CLIENT_SECRET")
    redirect_uri: str = Field(default="http://localhost:8000/auth/callback", description="OIDC重定向URI", env="OIDC__REDIRECT_URI")
...
class SecuritySettings(BaseSettings):
    """安全配置"""
    # JWT配置
    jwt_secret: str = Field(default_factory=lambda: secrets.token_urlsafe(32), description="JWT密钥", env="SECURITY__JWT_SECRET")
    jwt_algorithm: str = Field(default="HS256", description="JWT算法", env="SECURITY__JWT_ALGORITHM")
    jwt_expire_minutes: int = Field(default=30, description="JWT过期时间(分钟)", env="SECURITY__JWT_EXPIRE_MINUTES")
...
class Settings(BaseSettings):
    ...
    # 应用配置
    app_name: str = Field(default="PhoenixCoder", description="应用名称", env="APP_NAME")
    ...
    host: str = Field(default="0.0.0.0", description="服务主机", env="HOST")
    port: int = Field(default=8000, description="服务端口", env="PORT")
    ...
    log_format: str = Field(default="json", description="日志格式", env="LOG_FORMAT")
    ...
    # CORS配置
    cors_origins: List[str] = Field(default=["http://localhost:3000"], description="CORS允许的源", env="CORS_ORIGINS")
...
def get_settings() -> Settings:
    ...
    if _settings is None:
        # 直接创建子配置实例，避免嵌套环境变量问题
        database_settings = DatabaseSettings()
        redis_settings = RedisSettings()
        
        # 为OIDC和Security使用默认值，避免必填字段错误
        oidc_settings = OIDCSettings(
            issuer="http://localhost:8000",
            client_id="phoenixcoder-client",
