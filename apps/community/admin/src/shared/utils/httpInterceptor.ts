...
import { handleHttpError } from '@/shared/components/GlobalErrorHandler';
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (isLoginApi && isLoginPage && response.status === 400) {
            ...
            logger.debug('登录API 400错误，不拦截，让页面自己处理');
            return response;
          }
        ...
        logger.error('Fetch wrapper error:', fetchError);
    ...
    if (typeof dataProvider[method] === 'function') {
      enhancedProvider[method] = async (...args: unknown[]) => {
        try {
          ...
          logger.error(`DataProvider ${method} error:`, providerError);
