...
const SkillMap = ({ skillsData, onSkillSelect }: SkillMapProps) => {
    ...
    // 处理技能选择
    const handleSkillSelect = (_skill: SkillData) => {
        setSelectedSkill(_skill);
        if (onSkillSelect) {
            onSkillSelect(_skill);
        }
    };

    // 格式化雷达图数据
    const radarData = skillsData.map((_skill, _index) => ({
        name: _skill.name,
        value: _skill.value,
        description: _skill.description,
        category: _skill.category,
    }));

    // 提取技能类别和名称
    const categories = Array.from(
        new Set(skillsData.filter((_skill) => _skill.category).map((_skill) => _skill.category)),
    );
    // const _skillNames = skillsData.map((_skill) => _skill.name); // 暂时未使用
    ...
    return (
        <View className={styles.container} style={{ backgroundColor: currentTheme.bgColor }}>
            ...
            <View className={styles.chartContainer}>
                <ResponsiveContainer width={screenWidth} height={350}>
                ...
                {categories.map((category) => (
                    <View key={category} className={styles.categorySection}>
                        ...
                        <View className={styles.skillsRow}>
                            {skillsData
                                .filter((_skill) => _skill.category === category)
                                .map((_skill) => (
                                    <TouchableOpacity
                                        key={_skill.name}
                                        className={`${styles.skillCard} ${selectedSkill?.name === _skill.name ? styles.selected : ''}`}
                                        style={{
                                            backgroundColor:
                                        ...
                                        onClick={() => handleSkillSelect(_skill)}
                                    >
                                        <Text
                                            style={{
                                                color:
