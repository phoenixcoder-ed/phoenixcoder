...
async def update_learning_plan(
    ...
    try:
        ...
        updated_plan = LearningPlanResponse(
            ...
            target_skills=request.target_skills or ["Python", "数据结构"],
            start_date=request.start_date.isoformat() if request.start_date else datetime.now().date().isoformat(),
            end_date=request.end_date.isoformat() if request.end_date else (datetime.now() + timedelta(days=30)).date().isoformat(),
            daily_hours=request.daily_hours or 2.0,
            priority=request.priority or "medium",
            status=request.status or "active",
            progress=25.0,  # 保持原有进度
            total_study_hours=40.0,
            created_at=(datetime.now() - timedelta(days=5)).isoformat(),  # 保持原创建时间
            updated_at=datetime.now().isoformat()
        )
