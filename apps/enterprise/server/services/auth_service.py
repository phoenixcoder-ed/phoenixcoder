...
class PasswordService:
    ...
    def hash_password(password: str) -> str:
        """密码哈希"""
        salt = settings.security.jwt_secret[:16]  # 使用JWT密钥的前16位作为盐
    ...
    def validate_password_strength(password: str) -> Tuple[bool, str]:
        ...
        if len(password) < settings.security.password_min_length:
            return False, f"密码长度至少{settings.security.password_min_length}位"
...
class AuthService:
    ...
    async def _find_user(self, credentials: LoginCredentials) -> Optional[Dict[str, Any]]:
        ...
        if credentials.email:
            ...
            return await self.user_repository.get_by_phone(credentials.phone)
        elif credentials.username:
            return await self.user_repository.get_by_username(credentials.username)
        return None
