...
const ThemeToggle: React.FC<any> = ({ theme, onThemeChange }) => (
    <View className="theme-toggle">
        <AtButton size="small" onClick={() => onThemeChange(theme === 'dark' ? 'light' : 'dark')}>
            {theme === 'dark' ? '🌞' : '🌙'}
        </AtButton>
    </View>
);
...
class Growth extends Component<Props> {
    ...
    // 更新学习进度
    handleUpdateProgress = (_progress: number): void => {
        this.props.updateGrowthProgress(_progress).catch((err: Error) => {
            console.error('更新学习进度失败:', err);
        });
    };

    // 完成挑战
    handleCompleteChallenge = (_challengeId: string): void => {
        this.props.completeChallenge(_challengeId).catch((err: Error) => {
        ...
        return (
            <View className="growth-container">
                ...
                >
                    ...
                    <ThemeToggle theme={theme} onThemeChange={this.handleThemeChange} />
                </View>
