...
const ThemeToggle = ({ initialTheme = 'dark', onChange }: ThemeToggleProps): React.ReactNode => {
    const [_theme, setTheme] = useState<Theme>(initialTheme);
    const theme = _theme;
    ...
    useEffect(() => {
        ...
        if (storedTheme) {
            ...
            const _initialTheme = systemTheme as Theme;
            setTheme(_initialTheme);
            setIsDarkMode(_initialTheme === 'dark');
            applyTheme(_initialTheme);
            if (onChange) {
                onChange(_initialTheme);
            }
        }
