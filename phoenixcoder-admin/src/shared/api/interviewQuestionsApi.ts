...
export const getQuestions = async (
  ...
  try {
    ...
    throw new Error('获取题目列表失败: ' + (error instanceof Error ? error.message : '未知错误'));
  }
};
...
export const getQuestion = async (questionId: string): Promise<Question> => {
  try {
    ...
    throw new Error('获取题目详情失败: ' + (error instanceof Error ? error.message : '未知错误'));
  }
};
...
export const createQuestion = async (questionData: CreateQuestionRequest): Promise<Question> => {
  try {
    ...
    throw new Error('创建题目失败: ' + (error instanceof Error ? error.message : '未知错误'));
  }
};
...
export const updateQuestion = async (
  ...
  try {
    const response = await api.put<Question>(`/questions/${questionId}`, questionData);
    return response.data;
  } catch (error) {
    throw new Error('更新题目失败: ' + (error instanceof Error ? error.message : '未知错误'));
  }
};
...
export const deleteQuestion = async (questionId: string): Promise<void> => {
  try {
    ...
    throw new Error('删除题目失败: ' + (error instanceof Error ? error.message : '未知错误'));
  }
};