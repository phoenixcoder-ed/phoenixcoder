...
import { Box, Chip, IconButton, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Tooltip, TextField } from '@mui/material';
...
const ArticleTable: React.FC<ArticleTableProps> = ({ articles, onEdit, onDelete, onView }) => {
        ...
        <Table sx={{ minWidth: 650 }} aria-label="article table">
          ...
          <TableBody>
            {filteredArticles.map((article) => (
              ...
              >
                <TableCell component="th" scope="row" sx={{ maxWidth: '200px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {article.title}
                </TableCell>
                <TableCell>
                  <Chip
                ...
                <TableCell align="right">
                  <Tooltip title="查看">
                    <IconButton size="small" onClick={() => onView(article.id)} sx={{ color: theme.palette.text.secondary }}>
                      <Visibility fontSize="small" />
                    </IconButton>
                  </Tooltip>
                  <Tooltip title="编辑">
                    <IconButton size="small" onClick={() => onEdit(article)} sx={{ color: theme.palette.primary.main }}>
                      <Edit fontSize="small" />
                    </IconButton>
                  </Tooltip>
                  <Tooltip title="删除">
                    <IconButton size="small" onClick={() => onDelete(article.id)} sx={{ color: theme.palette.error.main }}>
